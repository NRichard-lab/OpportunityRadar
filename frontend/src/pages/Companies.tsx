...
export function Companies({ onViewCompanyJobs, onCompaniesChanged, onCompanyRefreshed, onCompanyDeleted, selectedCompanyName, refreshEnabled }: CompaniesProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(`${API_BASE}/companies-page?${parameters}`, { signal: controller.signal });
        if (!response.ok) throw new Error("Could not load companies.");
        const nextResult = await response.json() as CompanyPage;
        setResult(nextResult);
        if (nextResult.page !== page) setPage(nextResult.page);
      } catch (caught) {
        if (!(caught instanceof DOMException && caught.name === "AbortError")) {
          setPageError(caught instanceof Error ? caught.message : "Could not load companies.");
        }
      } finally {
  ...
  const saveCompany = async (form: CompanyFormData) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/companies${isEdit ? `/${editingCompany?.id}` : ""}`, {
        method: isEdit ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || result.message || "Could not save the company.");
      await onCompaniesChanged();
      setRefreshToken((current) => current + 1);
      setEditingCompany(undefined);
      setNotice(isEdit ? "Company updated." : "Company added.");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not save the company.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/companies/${deletingCompany.id}`, { method: "DELETE" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || result.message || "Could not delete the company.");
      await onCompanyDeleted(result.deletedJobIds || []);
      setRefreshToken((current) => current + 1);
      setDeletingCompany(null);
      setNotice("Company and related job data deleted.");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not delete the company.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const refreshCompany = async (company: Company) => {
    if (!refreshEnabled) {
      setRefreshResults((current) => ({ ...current, [company.id]: {
        status: "failed", companyName: company.name, companyMetadataChanged: false,
    ...
    try {
      const response = await fetch(`${API_BASE}/companies/${company.id}/refresh`, { method: "POST" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || `Could not refresh ${company.name}.`);
      setRefreshResults((current) => ({ ...current, [company.id]: result as CompanyRefreshResult }));
      await onCompanyRefreshed();
      setRefreshToken((current) => current + 1);
    } catch (caught) {
      const message = caught instanceof Error ? caught.message : `Could not refresh ${company.name}.`;
      setRefreshResults((current) => ({ ...current, [company.id]: {
        status: "failed", companyName: company.name, companyMetadataChanged: false,
        totalJobsDiscovered: 0, newJobs: 0, updatedJobs: 0, removedOrClosedJobs: 0,
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-400">{rangeLabel(result)}</p>
      ...
      />
      {pageError ? <div className="rounded-md border border-red-900 bg-red-950/40 px-4 py-3 text-sm text-red-300" role="alert">{pageError}</div> : null}
      <div className={`space-y-3 ${loading ? "opacity-60" : ""}`} aria-busy={loading}>
        ...
        {!loading && !result.items.length ? <Empty message="No companies match the current filters." /> : null}
      </div>
